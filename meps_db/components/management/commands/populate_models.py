...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--years", nargs="*", help="declare year to populate models with like '2016'",
        )
        parser.add_argument(
        ...
        for year in years:
            for model_key in model_list:
                ...
                if options["verbosity"] > 0:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Successfully populated {model_params_dict[model_key]['model_name']} Model with "
                            f"{len(model_objs) :,} {model_params_dict[model_key]['object_name']} from year {year}"
                        )
                    )
